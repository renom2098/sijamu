                        ...
                        <div class="col-12">
                            ...
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">Kelola Pengguna
                                    </li>
                                </ol>
                            </div>
                        </div>
                                        ...
                                        <div class="card">
                                            <div class="card-header border-bottom">
                                                <h4 class="card-title">Kelola Pengguna</h4>
                                                    <button onclick="add()" type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addData">Tambah Data Pengguna</button>
                                            </div>
...
<script>
    $(document).ready(function(){
        $('#table').DataTable({
            ...
            "serverSide": false,
            "paging": true,
            "ajax": "<?= site_url('admin/getData_pengguna'); ?>"
        })
    });
</script>
...
function add()
{
	$.post("<?= site_url("admin/viewAddDataPengguna"); ?>",{},function(data){
		$("#formAdd").html(data);
		$("#modal_edit").attr(
            "url",
            "<?= site_url("admin/insert_dataPengguna"); ?>"
        );
...
function edit(id)
{
	$.post("<?= site_url("admin/viewEditDataPengguna"); ?>",{id:id},function(data){
		$("#formEdit").html(data);
		$("#modal_edit").attr(
            "url",
            "<?= site_url("admin/update_dataPengguna"); ?>"
        );
...
// function review(id)
// {
// 	$.post("<?= site_url("admin/viewReviewDataPengguna"); ?>",{id:id},function(data){
...
// {
//     var url = "<?= site_url("admin/downloadPengguna/"); ?>" + id;
// 	$.post(url);
// }
...
function hapus(id,akun)
{
	swal({
		...
		if (willDelete) {
			...
			$.post("<?= site_url("admin/delete_dataPengguna"); ?>",{id:id},function(){
			reload_data();
			});
